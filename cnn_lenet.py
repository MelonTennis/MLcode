...
def pooling_layer_forward(input, layer):
  ...
  # inputData = np.reshape(input['data'], [h_in, w_in, c, batch_size])
  # outputData = np.reshape(output['data'], [h_out, w_out, c, batch_size])
  # for b in range(0, batch_size):
  #   for cc in range(0, c):
  #     for j in range(0, w_out):
  #       for i in range(0, h_out):
  #         outputData[i][j][cc][b] = max([inputData[r][l][cc][b] for r in range(stride*i, stride*i+k) for l in range(stride*j, stride*j+k)])
  # output['data'] = np.reshape(outputData, [h_out*w_out*c, batch_size])

  # data = input['data']
  # for b in range(batch_size):
  #   singlePic = data[:, b]
  #   for channel in range(c):
  #     singleChannel = singlePic[channel * h_in * w_in:(channel + 1) * h_in * w_in]
  #     paddedMatrix = np.zeros((h_in + 2 * pad, w_in + 2 * pad))
  #     for i in range(h_in):
  #       paddedMatrix[pad + i][pad:pad + w_in] = singleChannel[i * w_in:(i + 1) * w_in]
  #
  #     pooledMatrix = np.zeros((h_out, w_out))
  #     for i in range(0, h_out):
  #       for j in range(0, w_out):
  #         pooledMatrix[i][j] = np.amax(paddedMatrix[i * stride:i * stride + k, j * stride:j * stride + k])
  #     output['data'][:, b][channel * h_out * w_out:(channel + 1) * h_out * w_out] = pooledMatrix.flatten()
  # in_data = input['data'].reshape(h_in, w_in, c, batch_size)
  # out_data = output['data'].reshape(h_out, w_out, c, batch_size)
  #

  # for i in range(in_data.shape[3]):
  #   for j in range(in_data.shape[2]):
  #     m = 0
  #     f = 0
  #     while m < in_data.shape[1]:
  #       n = 0
  #       g = 0
  #       while n < in_data.shape[0]:
  #         tmp = []
  #         for a in range(k):
  #           for b in range(k):
  #             tmp.append(in_data[n + a][m + b][j][i])
  #         max_value = np.amax(tmp)
  #         out_data[g][f][j][i] = max_value
  #         n += stride
  #         g += 1
  #       m += stride
  #       f += 1
  # output['data'] = out_data.reshape(h_out * w_out * c, batch_size)
  ...
  for n in range(0, batch_size):
    ...
    col = im2col_conv(input_n, layer, h_out, w_out)
    col = np.reshape(col, (k*k, c, h_out * w_out))
    tmp_out = np.reshape(np.max(col, axis=0), (c, h_out*w_out))
    # tmp_out = np.reshape(tmp_out.T, (h_out, w_out, c))
    output['data'][:, n] = tmp_out.T.flatten()
  # implementation ends
...
def pooling_layer_backward(output, input, layer):
  ...
  for b in range(0, batch_size):
    ...
    col = im2col_conv(input_n, layer, h_out, w_out)
    col = np.reshape(col, (k*k, c, h_out*w_out))
    tmp_data_diff = np.reshape(output['diff'][:, b], (h_out * w_out, c))
    tmp_data_diff = np.tile(np.reshape(tmp_data_diff, (1, c, h_out*w_out)), (k*k, 1, 1))
    tmp_data_diff = np.reshape(tmp_data_diff, (k*k, c, h_out*w_out))
    tmp_data = np.reshape(output['data'][:, b], (h_out*w_out, c))
    tmp_data = np.tile(np.reshape(tmp_data.T, (1, c, h_out*w_out)), (k*k, 1, 1))
    tmp_data = np.reshape(tmp_data, (k*k, c, h_out*w_out))
    col_diff = (tmp_data == col) * tmp_data_diff
    im = col2im_conv(col_diff, input, layer, h_out, w_out)
    input_od[:, b] = im.flatten()
  ...
  # for b in range(batch_size):
  #   singlePic = data[:, b]
  #   singleOut = output['data'][:, b]
  #   singleDiff = output['diff'][:, b]
  #   for channel in range(c):
  #     singleChannelI = singlePic[channel * h_in * w_in:(channel + 1) * h_in * w_in]
  #     paddedMatrix = np.zeros((h_in + 2 * pad, w_in + 2 * pad))
  #     for i in range(h_in):
  #       paddedMatrix[pad + i][pad:pad + w_in] = singleChannelI[i * w_in:(i + 1) * w_in]
  #
  #     singleChannelO = singleOut[channel * h_out * w_out:(channel + 1) * h_out * w_out]
  #     matrixO = np.zeros((h_out, w_out))
  #     for i in range(h_out):
  #       matrixO[i][:] = singleChannelO[i * w_out:(i + 1) * w_out]
  #
  #     singleChannelD = singleDiff[channel * h_out * w_out:(channel + 1) * h_out * w_out]
  #     matrixD = np.zeros((h_out, w_out))
  #     for i in range(h_out):
  #       matrixD[i][:] = singleChannelD[i * w_out:(i + 1) * w_out]
  #
  #     padded_od = np.zeros((h_in + 2 * pad, w_in + 2 * pad))
  #
  #     for i in range(0, h_out):
  #       for j in range(0, w_out):
  #         for m in range(i * stride, i * stride + k):
  #           for n in range(j * stride, j * stride + k):
  #             if paddedMatrix[m][n] == matrixO[i][j]:
  #               padded_od[m][n] += matrixD[i][j]
  #     input_od[:, b][channel * h_in * w_in:(channel + 1) * h_in * w_in] = padded_od[pad:pad + h_in, pad:pad + w_in].flatten()

  h_in = input['height']
  w_in = input['width']
  stride = layer['stride']
  in_data = input['data'].reshape(h_in, w_in, c, batch_size)
  input_od = input_od.reshape(h_in, w_in, c, batch_size)
  out_data = output['data'].reshape(h_out, w_out, c, batch_size)
  tmp_diff = output['diff'].reshape(h_out, w_out, c, batch_size)
  for i in range(in_data.shape[3]):
    for j in range(in_data.shape[2]):
      m = 0
      f = 0
      while m < in_data.shape[1]:
        ...
        while n < in_data.shape[0]:
          for a in range(k):
            for b in range(k):
              if out_data[g][f][j][i] == in_data[n + a][m + b][j][i]:
                input_od[n + a][m + b][j][i] = tmp_diff[g][f][j][i]
          n += stride
          g += 1
        m += stride
        f += 1
...
def relu_backward(output, input, layer):
  ...
  # implementation begins
  # temp_data = input['data'].copy()
  # temp_data[temp_data > 0] = 1
  # input_od = temp_data*output['diff']
  for i in range(input_od.shape[0]):
    for j in range(input_od.shape[1]):
      if input['data'][i][j] > 0:
...
def inner_product_forward(input, layer, param):
  ...
  # w = param['w']
  # b = param['b']
  # tempOutput = np.zeros((batch_size, num))
  # tempinput = np.transpose(input['data'])
  # for bb in range(0, batch_size):
  #   tempOutput[bb] = np.dot(np.transpose(w), tempinput[bb]) + b
  # output['data'] = np.transpose(tempOutput)

  for b in range(batch_size):
    output['data'][:, b] = np.dot(np.transpose(param['w']), input['data'][:, b]) + np.transpose(param['b'])
  # implementation ends
...
def inner_product_backward(output, input, layer, param):
  ...
  param_grad['w'] = np.dot(input['data'], np.transpose(output['diff']))
...
def mlrloss(wb, X, y, K, prediction):
  ...
  if np.min(prob[y, np.arange(batch_size)]) < 0:
    print prob[y, np.arange(batch_size)]
    print np.sum(activation, axis=0)
    print wb
...
def sgd_momentum(w_rate, b_rate, mu, decay, params, param_winc, param_grad):
  ...
  # implementation begins
  # for l, para in params.items():
  #   lparam = params[l]
  #   lparam_winc = param_winc[l]
  #   lparam_grad = param_grad[l]
  #   gw = mu * lparam_winc['w'] + w_rate * (lparam_grad['w'] + decay * lparam['w'])
  #   gb = mu * lparam_winc['b'] + b_rate * (lparam_grad['b'])
  #   # gb = mu*lparam_winc['b']+b_rate*(lparam_grad['b']+decay*lparam['b'])
  #   neww = lparam['w'] - gw
  #   newb = lparam['b'] - gb
  #   params_[l]['w'] = neww
  #   params_[l]['b'] = newb
